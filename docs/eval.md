...
The bundled multi-shape gold set (`npm run eval:multi`) currently covers seven
...
service), and a **precision** case (an inventory service seeded with unrelated
decoy modules, gated on useful-context density) — verifying packet quality
across languages and repo shapes, not just one.
